...
| 기능 | 주입 | 확인 | 합격 |
|---|---|---|---|
| 타이밍 링 등급 | 황금 밴드에서의 거리 0.02 / 0.08 / 0.15 / 0.30 (비율) 에서 pointerdown — 창은 `BALANCE.capture` (perfect .045 · great .10 · good .17) | 등급이 PERFECT / GREAT / GOOD / MISS 로 단조 | 경계값 ±0.005 에서 뒤집히지 않음, 3회 반복 동일, 판정 시각은 `event.timeStamp` |
