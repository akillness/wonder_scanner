...
import { app, $, $$, esc, toast } from './shell.js';
import { state, save } from '../game/state.js';
import { SKILLS, SHAPES, EMOJI_PALETTE, ownsSkill, ownedSkills, toneCss, applyWarp, drawShape, drawEmoji, drawGlow, bakeEdit } from '../game/skills.js';
...
export function openEditor(momentId, onDone = () => {}) {
  getMoment(momentId).then(async (m) => {
    ...
    el.innerHTML = `<div class="sheet editor" onclick="event.stopPropagation()">
      <div class="row"><h3 class="grow">🧪 렌즈 스킬</h3><button class="btn sm ghost" id="edReset">↺ 원본</button></div>
      <canvas id="edCanvas"></canvas>
      <div class="chips left skills">${SKILLS.map(s => `<button class="pill ${ownsSkill(s.id) ? '' : 'locked'} ${s.id === skill ? 'on' : ''}" data-sk="${s.id}" title="${esc(s.desc)}">${s.icon} ${s.name}${ownsSkill(s.id) ? '' : ` 🔒Lv${s.unlockLevel}`}</button>`).join('')}</div>
      <div id="edPanel"></div>
    ...
    const panel = { emoji: '😄', size: 0.16, shape: 'spot', text: '', mode: 'bulge', strength: 0.5, tone: { brightness: 100, contrast: 100, saturate: 100, temp: 0 }, glow: { vignette: 0.6, sparkles: 40 }, hiddenEmoji: '🔭' };
    const renderPanel = () => { const P = $('#edPanel', el); const sk = skill;
      if (sk === 'emoji') P.innerHTML = `<div class="chips left">${EMOJI_PALETTE.map(e => `<button class="pill ${panel.emoji === e ? 'on' : ''}" data-e="${e}">${e}</button>`).join('')}</div><label>크기 <input type="range" id="eSize" min="6" max="40" value="${panel.size * 100}"/></label><small>사진을 탭해 붙이고, 드래그해 옮겨요. <button class="btn sm ghost" id="eUndo">↩ 하나 지우기</button></small>`;
      ...
      else if (sk === 'shape') P.innerHTML = `<div class="chips left">${SHAPES.map(s => `<button class="pill ${panel.shape === s.id ? 'on' : ''}" data-sh="${s.id}">${s.name}</button>`).join('')}</div><label>문구 (말풍선·폴라로이드) <input id="shText" maxlength="24" value="${esc(panel.text)}" placeholder="한 마디"/></label><small>사진을 탭해 위치 지정. <button class="btn sm ghost" id="shUndo">↩ 하나 지우기</button></small>`;
      else if (sk === 'warp') P.innerHTML = `<div class="chips left">${[['bulge', '볼록'], ['pinch', '오목'], ['swirl', '소용돌이']].map(([k, n]) => `<button class="pill ${panel.mode === k ? 'on' : ''}" data-w="${k}">${n}</button>`).join('')}</div><label>강도 <input type="range" id="wStr" min="10" max="100" value="${panel.strength * 100}"/></label><small>사진을 탭해 중심 지정. <button class="btn sm ghost" id="wClear">✕ 왜곡 해제</button></small>`;
      else if (sk === 'hidden') P.innerHTML = `<div class="chips left">${['🔭', '🐾', '💎', '👀', '🍀', '🫧'].map(e => `<button class="pill ${panel.hiddenEmoji === e ? 'on' : ''}" data-h="${e}">${e}</button>`).join('')}</div><div class="row"><button class="btn sm ghost grow" id="hHide">🫥 랜덤 위치에 숨기기</button><button class="btn sm ${edit.cover ? 'primary' : 'ghost'} grow" id="hCover">${edit.cover ? '🔓 가리기 해제' : '🔒 사진 가리기(스크래치)'}</button></div><small>숨긴 이모지는 회상할 때 "숨은 그림 찾기"가 되고, 찾으면 별가루 +15.</small>`;
      else if (sk === 'glow') P.innerHTML = `<label>비네트 <input type="range" id="gVig" min="0" max="90" value="${panel.glow.vignette * 100}"/></label><label>반짝이 <input type="range" id="gSp" min="0" max="120" value="${panel.glow.sparkles}"/></label><div class="row"><button class="btn sm ghost grow" id="gOn">✨ 적용</button><button class="btn sm ghost" id="gOff">✕</button></div>`;
      ...
      $$('[data-h]', P).forEach(b => b.onclick = () => { panel.hiddenEmoji = b.dataset.h; renderPanel(); }); $('#hHide', P) && ($('#hHide', P).onclick = () => { edit.hidden = { emoji: panel.hiddenEmoji, x: 0.08 + Math.random() * 0.84, y: 0.08 + Math.random() * 0.84, found: 0 }; render(); toast(`${panel.hiddenEmoji} 어딘가에 숨겼어요`, 1500); }); $('#hCover', P) && ($('#hCover', P).onclick = () => { edit.cover = !edit.cover; renderPanel(); render(); });
    ...
    $$('[data-sk]', el).forEach(b => b.onclick = () => { if (!ownsSkill(b.dataset.sk)) { fx.denied(); toast(`🔒 Lv${SKILLS.find(s => s.id === b.dataset.sk).unlockLevel}에 해금 · 상점에서 구매 가능`, 2000); return; } skill = b.dataset.sk; $$('[data-sk]', el).forEach(x => x.classList.toggle('on', x === b)); renderPanel(); fx.blip(); });
    ...
    $('#edSave', el).onclick = async (e) => { e.target.disabled = true; e.target.textContent = '굽는 중…';
      ...
      close(); fx.chest(); if (r?.leveled) toast(`🌱 스킬로 다듬어 「${r.leveled.icon} ${r.leveled.name}」로 자랐어요! ✨+${r.leveled.reward.dust}`, 3200, 'quest'); else toast(`🧪 스킬 ${used.length}종 적용 · 저장`, 1800); onDone(r?.m); };
