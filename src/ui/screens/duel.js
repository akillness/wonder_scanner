import { register, go, app, $, esc, toast } from '../shell.js';
...
register('duel', async (myId, otherId = 'shadow') => {
  ...
  const card = (m, w, u, side, s) => `<div class="duel-card ${side} ${m.shadow ? 'shadow' : ''} ${m.variant ? 'variant' : ''}" style="--r-color:${RARITY[w.rarity].color}"><div class="hp"><i id="hp-${side}" style="width:100%"></i></div><img src="${u}" alt=""/><b>${m.shadow ? '👤 ' : ''}${w.emoji} ${esc(m.shadow ? m.name : w.name)}</b><small>${STAGES[m.stage || 0].icon} ${m.grade} ${m.variant ? '✨' : ''}</small><div class="st">${['신비', '타이밍', '성장'].map(k => `<span>${k} <b>${s[k]}</b></span>`).join('')}</div></div>`;
  app.innerHTML = `
  <section class="screen duel-screen">
    <header><button class="btn icon ghost" id="back">←</button><h2>⚔️ 추억 대결</h2><span class="pill" id="round">READY</span></header>
    <div class="arena">${card(a, wa, ua, 'a', sim.sa)}<div class="vs">VS</div>${card(b, wb, ub, 'b', sim.sb)}</div>
    <div class="log" id="log"></div>
    <div class="actions" id="acts"><button class="btn primary big" id="fight">⚔️ 대결 시작</button></div>
  </section>`;
  ...
  $('#fight').onclick = async () => {
    ...
    const won = sim.winner === 'a'; $('#round').textContent = won ? '🏆 WIN' : '💀 LOSE';
    ...
    $('#acts').innerHTML = `<div class="rewards"><span>${won ? '승리' : '패배'} ✨ <b>+${dust}</b></span>${bonus ? `<span>도전장 보상 ✨ <b>+${bonus}</b></span>` : ''}${ev?.leveled ? `<span>${ev.leveled.icon} ${ev.leveled.name}로 진화!</span>` : '<span>🌱 성장 +1</span>'}</div>
      <div class="row"><button class="btn primary grow" id="again">🔁 다른 상대</button><button class="btn ghost grow" id="toAlbum">📸 앨범</button></div>`;
