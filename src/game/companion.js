...
export function advise(ctx = {}) {
  ...
  const rules = [
    [() => ev && !ev.seen, () => ({ icon: '⚡', text: `사건 발생! ${ev.title} — ${ev.desc}`, action: 'event' })],
    [() => state.prismTokens > 0 && ctx.screen === 'scan' && !ctx.armed, () => ({ icon: '✨', text: `프리즘 토큰이 ${state.prismTokens}개 있어. 희귀한 게 보이면 장착해서 변이체로 만들자.`, action: 'arm' })],
    [() => state.boost > 0 && ctx.screen === 'scan', () => ({ icon: '⚡', text: '공명 부스트가 걸려 있어. 이번엔 두 배로 빨리 찰 거야.' })],
    [() => state.dust >= 320 && state.prismTokens === 0, () => ({ icon: '🏪', text: `별가루가 ${state.dust}이나 쌓였어. 상점에서 프리즘 토큰을 사 두는 건 어때?`, action: 'shop' })],
    [() => nm && nm.at - ownedCount() <= 2, () => ({ icon: '🎁', text: `보물상자 「${nm.title}」까지 ${nm.at - ownedCount()}종! 코앞이야.`, action: 'codex' })],
    [() => c.missing.length && c.remain <= 3, () => ({ icon: c.chapter.icon, text: `${c.chapter.title}는 ${c.remain}개만 더 찍으면 이야기가 열려. ${c.missing.slice(0, 2).map(l => WONDERS[l].emoji + ' ' + l).join(', ')} 어딨는지 알아?`, action: 'codex' })],
    [() => ctx.uncaptioned > 0, () => ({ icon: '✏️', text: `캡션이 없는 추억이 ${ctx.uncaptioned}장 있어. 한 줄만 적어도 자라기 시작해.`, action: 'album' })],
    [() => state.quests.items.some(q => !q.done && q.type === 'spirits') && ctx.screen === 'scan', () => ({ icon: '🫧', text: '정령 의뢰가 남았어. 폰을 천천히 돌려 봐, 시야 밖에 숨어 있을지도.' })],
    [() => hour() >= 22 || hour() < 5, () => ({ icon: '🌙', text: '밤이네. 조명이 어두우면 인식이 흔들려 — 램프 하나만 켜 줘.' })],
    [() => hour() >= 11 && hour() <= 13, () => ({ icon: '🍳', text: '점심시간! 부엌·식탁 원더 21종이 지금 가장 잘 보일 때야.' })],
    [() => state.streak.days >= 3, () => ({ icon: '🔥', text: `${state.streak.days}일 연속이라 XP ×${(1 + Math.min(state.streak.days - 1, 5) * 0.1).toFixed(1)}. 오늘 전설을 노려볼까?` })],
    [() => true, () => ({ icon: '🔮', text: ['세상은 원더로 가득한데, 사람들은 그냥 "컵"이라고 불러.', '가만히 들고 있는 게 요령이야. 손이 아니라 눈으로 잡는 거지.', '난 렌즈 속에 살아. 네가 보는 걸 나도 봐.'][Math.floor(Math.random() * 3)] })],
  ];
...
export const EVENTS = [
  ...
  { id: 'errand', title: '루페의 부탁', desc: (p) => `${p.emoji} ${p.label}을(를) 찍어 와 줘 — 보상 ✨120 · 프리즘 1`, mod: { errand: true } },
  { id: 'shadow', title: '그림자 도전장', desc: () => '그림자 추억이 대결을 걸어왔어 — 앨범에서 ⚔️ 대결 승리 시 ✨150', mod: { duelBonus: 150 } },
];
...
export function onDiscoverEvent(label) {
  ...
  if (e.mod.errand && e.params.label === label) { e.done = true; state.dust += 120; state.prismTokens += 1; save(); return { title: e.title, text: '부탁 완료! ✨120 · 프리즘 토큰 1' }; }
  return null;
}
