...
- 장소 데이터: `VITE_GOOGLE_MAPS_KEY`가 있으면 **Google Places API (New)**, 없으면 **OpenStreetMap Nominatim** (키 불필요, 1초 간격 순차 조회).
- **현재 위치 기준 (v8.3)**: 카테고리마다 최대 40곳을 받아 **거리순**으로 정렬하고, 800m 안에 없는 카테고리만 2km로 넓혀 다시 찾는다. 거리·방위는 반올림 전 실제 좌표로 계산하고, 저장은 100m 단위 좌표만 1시간.
- **이동하면 다시 찾기**: 캐시 기준점에서 250m 넘게 움직이면 새로 검색하고, 그 안이면 거리만 현재 위치로 다시 계산한다. 위치 권한을 이미 허용했다면 권한 창 없이 타이틀·촬영지 화면에서 자동 갱신.
- 결과는 카테고리가 도착하는 대로 점진 표시되고, 헤더에 위치 정확도(±m)가 뜬다. 실측: 강남역 가장 가까운 카페 71m(v8.2는 244m), 동탄 5종 추천.
...
├─ geo/       provider.js · nominatim.js · google.js · osm.js (주변 촬영지)
├─ i18n/      index.js(한/영 런타임) · en.js(영어 사전, 지연 로드)
...
| 항목 | 동작 |
|---|---|
| 첫 방문 | 브라우저 언어를 따른다 (한국어면 한국어, 그 외 영어) · URL `?lang=en` / `?lang=ko` 도 된다 |
| 전환 | 타이틀 우상단 **EN / 한** 버튼, 또는 프로필 → 설정 → **언어 · Language** |
| 저장 | 기기(localStorage)에 저장 · 게임 진행·저장 데이터는 언어와 무관 |
| 번역하지 않는 것 | 사용자 콘텐츠(캡션·탐험가 이름)와 장소명 |

구현: 한국어 소스는 그대로 두고, 영어일 때만 사전(`src/i18n/en.js`, 지연 로드 37KB gzip)으로 화면 텍스트·속성·캔버스 글자·대화상자를 바꾼다. `npm run i18n:check` 가 새 한국어 문자열의 번역 누락을 보여 준다.
...
<div align="center"><sub>Sprint 1 (4h MVP) → Sprint 8 (AR · 추억 · 소셜 · NPC · 스킬 · 아이 게이지 · 위치 추천 · 정밀도 · 한/영) · 2026-09-23</sub></div>
